...
class SubscriptionDetails(BaseModel):
    ...
    status: StrictStr
    coupons: list[SubscriptionDetailsCouponsInner] | None = None
    discounts: list[SubscriptionDetailsDiscountsInner] | None = None
    trial_info: SubscriptionDetailsTrialInfo | None = Field(default=None, alias="trialInfo")
    ...
    def status_validate_enum(self, value):
        ...
        if value not in set(["active", "past_due", "canceled", "trialing"]):
            raise ValueError(
                "must be one of enum values ('active', 'past_due', 'canceled', 'trialing')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "plan": SubscriptionDetailsPlan.from_dict(obj["plan"])
                if obj.get("plan") is not None
                else None,
                "seats": SubscriptionDetailsSeats.from_dict(obj["seats"])
                if obj.get("seats") is not None
                else None,
                "renewal": SubscriptionDetailsRenewal.from_dict(obj["renewal"])
                if obj.get("renewal") is not None
                else None,
                "status": obj.get("status"),
                "coupons": [
                    SubscriptionDetailsCouponsInner.from_dict(_item) for _item in obj["coupons"]
                ]
                if obj.get("coupons") is not None
                else None,
                "discounts": [
                    SubscriptionDetailsDiscountsInner.from_dict(_item) for _item in obj["discounts"]
                ]
                if obj.get("discounts") is not None
                else None,
                "trialInfo": SubscriptionDetailsTrialInfo.from_dict(obj["trialInfo"])
                if obj.get("trialInfo") is not None
                else None,
