...
class TeamOverview(BaseModel):
    ...
    top_users: Annotated[list[TeamOverviewTopUsersInner], Field(max_length=10)] | None = Field(
        default=None, description="Most active users in the tenant", alias="topUsers"
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "usersByRole": TeamOverviewUsersByRole.from_dict(obj["usersByRole"])
                if obj.get("usersByRole") is not None
                else None,
                "pendingInvites": TeamOverviewPendingInvites.from_dict(obj["pendingInvites"])
                if obj.get("pendingInvites") is not None
                else None,
                "recentActivity": TeamOverviewRecentActivity.from_dict(obj["recentActivity"])
                if obj.get("recentActivity") is not None
                else None,
                "topUsers": [
                    TeamOverviewTopUsersInner.from_dict(_item) for _item in obj["topUsers"]
                ]
                if obj.get("topUsers") is not None
                else None,
