...
class LearnerAnalytics(BaseModel):
    ...
    range: StrictStr
    total_learning_time: StrictInt = Field(
        description="Total learning time in minutes", alias="totalLearningTime"
    )
    completion_rate: (
        Annotated[float, Field(le=100, strict=True, ge=0)]
        | Annotated[int, Field(le=100, strict=True, ge=0)]
    ) = Field(alias="completionRate")
    streak_days: StrictInt = Field(alias="streakDays")
    courses_started: StrictInt | None = Field(default=None, alias="coursesStarted")
    courses_completed: StrictInt | None = Field(default=None, alias="coursesCompleted")
    average_session_length: StrictInt | None = Field(
        default=None, description="Average session length in minutes", alias="averageSessionLength"
    )
    activity_data: list[LearnerAnalyticsActivityDataInner] = Field(alias="activityData")
    subject_breakdown: list[LearnerAnalyticsSubjectBreakdownInner] | None = Field(
        default=None, alias="subjectBreakdown"
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "activityData": [
                    LearnerAnalyticsActivityDataInner.from_dict(_item)
                    for _item in obj["activityData"]
                ]
                if obj.get("activityData") is not None
                else None,
                "subjectBreakdown": [
                    LearnerAnalyticsSubjectBreakdownInner.from_dict(_item)
                    for _item in obj["subjectBreakdown"]
                ]
                if obj.get("subjectBreakdown") is not None
                else None,
            }
        )
        return _obj
