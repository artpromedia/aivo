...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5432/ink_db",
        alias="DATABASE_URL"
    )
    ...
    s3_bucket_name: str = Field(
        default="aivo-ink-storage", alias="S3_BUCKET_NAME"
    )
    ...
    event_service_url: str = Field(
        ...
        alias="EVENT_SERVICE_URL"
    )
    enable_events: bool = Field(default=True, alias="ENABLE_EVENTS")

    # Security and validation
    max_strokes_per_request: int = Field(
        default=1000, alias="MAX_STROKES_PER_REQUEST"
    )
    max_points_per_stroke: int = Field(
        default=10000, alias="MAX_POINTS_PER_STROKE"
    )
    session_timeout_minutes: int = Field(
        default=60, alias="SESSION_TIMEOUT_MINUTES"
    )

    # Content moderation
    enable_consent_gate: bool = Field(
        default=True, alias="ENABLE_CONSENT_GATE"
    )
