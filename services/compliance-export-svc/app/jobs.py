...
DATABASE_URL = os.getenv(
    "DATABASE_URL", 
    "postgresql+asyncpg://user:password@localhost/compliance_db"
)
...
class ComplianceExportProcessor:
    ...
    def __init__(self):
        ...
        self.export_base_path = Path(
            os.getenv("COMPLIANCE_EXPORT_PATH", "/tmp/compliance-exports")
        )
        ...
        async with AsyncSessionLocal() as db_session:
            try:
                ...
                await self._handle_export_failure(
                    db_session, job_id, str(e), user_id
                )
                raise
    ...
    async def _process_edfacts_export(
        ...
        if export_type == "student":
            result = await exporter.export_student_data(
                job, output_path, school_year, district_id
            )
        elif export_type == "assessment":
            result = await exporter.export_assessment_data(
                job, output_path, school_year, 
                export_params.get("assessment_type"), district_id
            )
    ...
    async def _process_calpads_export(
        ...
        if export_type == "senr":
            result = await exporter.export_senr_data(
                job, output_path, school_year, district_code
            )
        elif export_type == "sass":
            result = await exporter.export_sass_data(
                job, output_path, school_year,
                export_params.get("test_type"), district_code
            )
        elif export_type == "sdis":
            result = await exporter.export_sdis_data(
                job, output_path, school_year, district_code
            )
        else:
    ...
    async def _encrypt_export_file(
        ...
        encrypted_size = encryption_manager.encrypt_file(
            original_path, encrypted_path, key_data
        )
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                processor.process_export_job(
                    job_uuid, format_type, export_params, user_id
                )
            )
            return result
        finally:
...
def validate_export_data(
    ...
    async def _validate():
        async with AsyncSessionLocal() as db_session:
            if format_type.lower() == "edfacts":
                exporter = EDFactsExporter(db_session)
                return await exporter.validate_export_data(
                    data_type, school_year, district_id
                )
            elif format_type.lower() == "calpads":
                exporter = CALPADSExporter(db_session)
                return await exporter.validate_calpads_data(
                    data_type, school_year, district_id
                )
            else:
