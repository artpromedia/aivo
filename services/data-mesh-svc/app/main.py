...
class DataProduct(BaseModel):
    ...
    def validate_privacy_for_domain(cls, v, values):
        if (
            "domain" in values
            and values["domain"] == EducationalDomain.SPECIAL_EDUCATION
            and v not in [PrivacyLevel.HIGHLY_SENSITIVE, PrivacyLevel.SPECIAL_EDUCATION]
        ):
...
async def register_data_product(product: DataProduct, auth: dict = Depends(verify_educational_access)):
    """Register a new educational data product with FERPA compliance validation"""

    # Validate educational domain requirements
    if product.domain == EducationalDomain.SPECIAL_EDUCATION and not product.idea_compliance:
        raise HTTPException(status_code=400, detail="Special education data products must be IDEA compliant")

    # Ensure FERPA compliance for student data
    if (
        product.privacy_level in [PrivacyLevel.STUDENT_IDENTIFIABLE, PrivacyLevel.HIGHLY_SENSITIVE]
        and not product.ferpa_compliance
    ):
        raise HTTPException(status_code=400, detail="Student identifiable data must be FERPA compliant")
...
async def discover_data_products(
    domain: EducationalDomain | None = None,
...
async def get_product_schema(
    product_id: str, version: str | None = None, auth: dict = Depends(verify_educational_access)
):
    ...
    schema = {
        ...
        "compliance_requirements": {"ferpa": product.ferpa_compliance, "idea": product.idea_compliance},
    }
...
async def create_governance_policy(policy: GovernancePolicy, auth: dict = Depends(verify_educational_access)):
    ...
    if policy.domain == EducationalDomain.SPECIAL_EDUCATION:
        ...
        for required in required_rules:
            if required not in policy_rule_types:
                raise HTTPException(status_code=400, detail=f"Special education policies must include {required} rule")
...
async def get_compliance_status(
    district_id: str, domain: EducationalDomain | None = None, auth: dict = Depends(verify_educational_access)
):
    ...
    idea_compliant = len(
        [p for p in district_products if p.idea_compliance or p.domain != EducationalDomain.SPECIAL_EDUCATION]
    )
...
def calculate_overall_compliance_score(products: list[DataProduct]) -> float:
    ...
    for product in products:
        ...
        if (
            product.privacy_level in [PrivacyLevel.STUDENT_IDENTIFIABLE, PrivacyLevel.HIGHLY_SENSITIVE]
            or product.privacy_level == PrivacyLevel.SPECIAL_EDUCATION
            and product.domain == EducationalDomain.SPECIAL_EDUCATION
        ):
            score += 30.0
    ...
    for domain in EducationalDomain:
        ...
        if domain_products:
            breakdown[domain.value] = {
                ...
                "ferpa_compliance_rate": len([p for p in domain_products if p.ferpa_compliance])
                / len(domain_products)
                * 100,
            ...
            if domain == EducationalDomain.SPECIAL_EDUCATION:
                breakdown[domain.value]["idea_compliance_rate"] = (
                    len([p for p in domain_products if p.idea_compliance]) / len(domain_products) * 100
                )
...
def generate_compliance_recommendations(products: list[DataProduct]) -> list[str]:
    ...
    non_ferpa = [
        p
        for p in products
        if not p.ferpa_compliance
        and p.privacy_level in [PrivacyLevel.STUDENT_IDENTIFIABLE, PrivacyLevel.HIGHLY_SENSITIVE]
    ]
    if non_ferpa:
    ...
    public_sped = [
        ...
        if p.privacy_level not in [PrivacyLevel.HIGHLY_SENSITIVE, PrivacyLevel.SPECIAL_EDUCATION]
    ]
    if public_sped:
...
async def create_analytics_pipeline(pipeline: AnalyticsPipeline, auth: dict = Depends(verify_educational_access)):
    ...
    if len(pipeline.source_domains) > 1:
        if not pipeline.privacy_preserving:
            raise HTTPException(status_code=400, detail="Cross-domain analytics must be privacy preserving")

        if EducationalDomain.SPECIAL_EDUCATION in pipeline.source_domains and not pipeline.student_consent_required:
            raise HTTPException(
                status_code=400,
...
async def get_domain_insights(
    ...
    domain_products = [p for p in data_products.values() if p.domain == domain and p.district_id == district_id]
...
def generate_educational_insights(
    domain: EducationalDomain, products: list[DataProduct], time_period: str
) -> dict[str, Any]:
        ...
        base_insights.update(
            {
                "assessment_insights": {
                    ...
                    "performance_gaps": {"demographic_gaps": "narrowing", "achievement_gap_reduction": 1.4},
                    "early_warning_indicators": {"at_risk_students": 147, "intervention_recommended": 89},
                }
            }
        )
        ...
        base_insights.update(
            {
                "attendance_insights": {
                    ...
                    "attendance_correlation": {"with_achievement": 0.73, "with_behavior": 0.68},
        ...
        base_insights.update(
            {
                "special_education_insights": {
                    ...
                    "service_delivery": {"service_hours_completed": "96.4%", "related_services_utilization": "89.2%"},
                    "compliance_metrics": {"iep_meeting_timeliness": "98.7%", "evaluation_compliance": "99.1%"},
                }
            }
        )
...
async def get_metrics():
    ...
    return {
        ...
        "educational_metrics": {
            "districts_served": len({p.district_id for p in data_products.values()}),
            "domains_active": len({p.domain for p in data_products.values()}),
            "privacy_levels_managed": len({p.privacy_level for p in data_products.values()}),
