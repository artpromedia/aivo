...
from .enums import SubjectType, LevelType, SessionStatus, QuestionType, AnswerResult
from .schemas import Question, AnswerEvaluation, SubjectLevelResult
...
class AssessmentSession:
    ...
    def __init__(self, user_id: str, subject: SubjectType):
    ...
    def is_complete(self) -> bool:
        ...
        return (
            self.status == SessionStatus.COMPLETED or
            len(self.questions_answered) >= settings.max_questions or
            self.confidence >= settings.convergence_threshold
        )
    ...
    def should_continue(self) -> bool:
        ...
        return (
            ...
            len(self.questions_answered) < settings.max_questions
        )
...
class QuestionBank:
    ...
    def get_question(self, subject: SubjectType, level: LevelType, exclude_ids: List[str] = None) -> Optional[Question]:
        ...
        available_questions = [
            q for q in self._questions[subject]
...
class AssessmentEngine:
    ...
    def start_assessment(self, user_id: str, subject: SubjectType) -> Tuple[AssessmentSession, Question]:
        ...
        logger.info(f"Started assessment session {session.session_id} for user {user_id} in {subject}")
        return session, first_question
    
    def submit_answer(self, session_id: str, question_id: str, answer: any) -> Tuple[AnswerEvaluation, Optional[Question], bool]:
        ...
        result = AnswerResult.CORRECT if str(answer).lower() == str(correct_answer).lower() else AnswerResult.INCORRECT
    ...
    def get_baseline_report(self, session_id: str) -> SubjectLevelResult:
        ...
        logger.info(f"Generated baseline report for session {session_id}: {result.level} with {result.confidence:.2f} confidence")
        return result
    
    def _update_level_estimate(self, session: AssessmentSession, question_id: str, result: AnswerResult):
        ...
        level_values = {LevelType.L0: 0, LevelType.L1: 1, LevelType.L2: 2, LevelType.L3: 3, LevelType.L4: 4}
        ...
        if total_questions >= settings.min_questions:
            ...
            session.confidence = min(1.0, accuracy * 0.8 + consistency_bonus + (total_questions * 0.05))
    
    def _get_next_question(self, session: AssessmentSession) -> Optional[Question]:
        ...
        next_question = self.question_bank.get_question(
            session.subject,
            session.current_level_estimate,
