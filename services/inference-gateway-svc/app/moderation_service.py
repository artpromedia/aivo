...
class ModerationService:
    ...
    async def moderate_content(
        self, content: str, threshold: float | None = None, model: str | None = None
        ...
        try:
            ...
            response = await self.client.moderations.create(input=content, model=model)
    ...
    async def moderate_batch(
        self, contents: list[str], threshold: float | None = None, model: str | None = None
    ...
    def get_moderation_summary(
        self, results: list[tuple[ModerationResult, dict[str, float]]]
    ) -> dict[str, Any]:
        """
        ...
        if all_scores:
            ...
            for category in categories:
                avg_scores[category] = sum(scores.get(category, 0) for scores in all_scores) / len(
                    all_scores
                )
