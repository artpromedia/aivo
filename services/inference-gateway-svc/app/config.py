...
class Settings(BaseSettings):
    ...
    moderation_threshold: float = Field(
        default=0.85, 
        description="Content moderation threshold",
        ge=0.0,
        le=1.0
    )
    ...
    pii_anonymization_enabled: bool = Field(
        default=True, 
        description="Enable PII anonymization"
    )
    supported_languages: List[str] = Field(
        default=["en"], 
        description="Supported languages for PII detection"
    )
