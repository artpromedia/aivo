...
class OpenAIService:
        ...
        try:
            ...
            for i, choice in enumerate(response.choices):
                ...
                if not request.skip_pii_scrubbing and settings.pii_anonymization_enabled:
                    gen_text, gen_entities, gen_scrubbed = await pii_service.process_text(
                        generated_text
                    )
                    if gen_scrubbed:
    ...
    async def generate_embeddings(self, request: EmbeddingRequest) -> EmbeddingResponse:
        ...
        try:
            ...
            for i, embedding in enumerate(response.data):
                data.append(
                    EmbeddingData(object="embedding", index=i, embedding=embedding.embedding)
                )
    ...
    async def moderate_content(self, request: ModerationRequest) -> ModerationResponse:
        ...
        try:
            ...
            response = self.client.moderations.create(input=texts, model=model)
            ...
            return ModerationResponse(id=response.id, model=response.model, results=results)
