...
class TestGenerationAPI:
    ...
    async def test_generate_text_success(
        self, client: AsyncClient, sample_generate_request: dict, mock_openai_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_text_with_pii(self, client: AsyncClient, mock_openai_client):
        ...
        with patch("app.openai_service.OpenAI", return_value=mock_openai_client):
            ...
            assert data["pii_scrubbed"]
            assert len(data["pii_entities"]) > 0

    @pytest.mark.asyncio
    async def test_generate_text_blocked_content(
        self, client: AsyncClient, mock_flagged_moderation
    ):
        """Test text generation with blocked content."""
        request_data = {"prompt": "Generate harmful content with hate speech", "max_tokens": 50}
    ...
    async def test_generate_text_with_educational_context(
        self, client: AsyncClient, mock_openai_client
    ):
...
class TestEmbeddingAPI:
    ...
    async def test_generate_embeddings_success(
        self, client: AsyncClient, sample_embedding_request: dict, mock_openai_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_embeddings_batch(self, client: AsyncClient, mock_openai_client):
        """Test batch embedding generation."""
        request_data = {"input": ["Text 1", "Text 2", "Text 3"], "model": "text-embedding-ada-002"}
    ...
    @pytest.mark.asyncio
    async def test_generate_embeddings_with_pii(self, client: AsyncClient, mock_openai_client):
        """Test embedding generation with PII scrubbing."""
        request_data = {"input": "Contact teacher at teacher@school.edu for help"}
...
class TestModerationAPI:
    ...
    async def test_moderate_content_safe(
        self, client: AsyncClient, sample_moderation_request: dict, mock_openai_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_moderate_content_harmful(self, client: AsyncClient, mock_flagged_moderation):
        """Test moderation of harmful content."""
        request_data = {"input": "This contains hate speech and violence", "threshold": 0.8}
    ...
    @pytest.mark.asyncio
    async def test_moderate_content_batch(self, client: AsyncClient, mock_openai_client):
        """Test batch content moderation."""
        request_data = {"input": ["Safe content 1", "Safe content 2", "Safe content 3"]}
...
class TestIntegrationWorkflows:
    ...
    @pytest.mark.asyncio
    async def test_complete_generation_workflow(self, client: AsyncClient, mock_openai_client):
        ...
        with patch("app.openai_service.OpenAI", return_value=mock_openai_client):
            ...
            assert data["moderation_result"] == "passed"
            assert data["pii_detected"]  # Email detected
            assert data["pii_scrubbed"]  # Email scrubbed
            assert data["context"]["subject"] == "Science"
            assert len(data["choices"]) == 1

    @pytest.mark.asyncio
    async def test_policy_enforcement_workflow(self, client: AsyncClient, mock_flagged_moderation):
        """Test policy enforcement blocks harmful content."""
        request_data = {
            "prompt": "Generate content that promotes violence and hate",
            "context": {"subject": "Ethics", "grade_level": "High School"},
        }
