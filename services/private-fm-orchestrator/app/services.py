...
from .schemas import PrivateBrainInstanceUpdate, PrivateBrainRequestCreate
...
class PrivateBrainOrchestrator:
    ...
    async def request_private_brain(
        self, request_data: PrivateBrainRequestCreate
        ...
        result = await self.db.execute(
            select(PrivateBrainInstance).where(PrivateBrainInstance.learner_id == learner_id)
        )
        ...
        if existing_instance:
            ...
            existing_instance.last_request_at = datetime.now(UTC)

            await self.db.commit()
            await self.db.refresh(existing_instance)

            logger.info(
                f"Existing private brain instance found for learner {learner_id}, count: {existing_instance.request_count}"
            )
    ...
    async def get_status(self, learner_id: int) -> PrivateBrainInstance | None:
    ...
    async def update_instance(
        self, instance_id: int, update_data: PrivateBrainInstanceUpdate
    ) -> PrivateBrainInstance | None:
        """Update a private brain instance (internal use)."""
        result = await self.db.execute(
            select(PrivateBrainInstance).where(PrivateBrainInstance.id == instance_id)
        )
        ...
        if (
            update_data.status == PrivateBrainStatus.READY
            and instance.status != PrivateBrainStatus.READY
        ):
        ...
        logger.info(
            f"Updated private brain instance {instance_id}: {update_data.model_dump(exclude_unset=True)}"
        )
        return instance
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                await orchestrator._update_instance_status(
                    instance_id, PrivateBrainStatus.CLONING, "Cloning private brain model..."
                )
            ...
            try:
                async with AsyncSessionLocal() as session:
                    orchestrator = PrivateBrainOrchestrator(session)
                    await orchestrator._update_instance_status(
                        instance_id, PrivateBrainStatus.ERROR, error_message=str(e)
                    )
            except Exception as cleanup_error:
                logger.error(
                    f"Failed to update error status for instance {instance_id}: {cleanup_error}"
                )

    async def _update_instance_status(
        self, instance_id: int, status: PrivateBrainStatus, error_message: str | None = None
    ) -> None:
        """Helper to update instance status."""
        update_data = PrivateBrainInstanceUpdate(status=status, error_message=error_message)
