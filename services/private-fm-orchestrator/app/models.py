...
class PrivateBrainInstance(Base):
    ...
    ns_uid: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
    checkpoint_hash: Mapped[str | None] = mapped_column(String, nullable=True)

    # Status tracking
    status: Mapped[PrivateBrainStatus] = mapped_column(
        String, nullable=False, default=PrivateBrainStatus.PENDING
    )
...
class PrivateBrainRequest(Base):
    ...
    # Request metadata
    request_source: Mapped[str | None] = mapped_column(String, nullable=True)  # e.g., "learner-svc"
