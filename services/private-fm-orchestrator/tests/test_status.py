...
class TestPrivateBrainStatus:
    ...
    async def test_get_status_not_found(
        ...
        assert "not found" in response_data["detail"].lower() or "no private brain" in response_data["detail"].lower()
    ...
    async def test_status_transitions(
        ...
        updated = await orchestrator.update_instance(
            instance.id,
            PrivateBrainInstanceUpdate(status=PrivateBrainStatus.CLONING)
        )
    ...
    async def test_error_status(
        ...
        updated = await orchestrator.update_instance(
            instance.id,
            PrivateBrainInstanceUpdate(
                status=PrivateBrainStatus.ERROR,
    ...
    @pytest.mark.asyncio
    async def test_async_cloning_process(
        self, 
        db_session: AsyncSession,
        sample_learner_id: int
    ):
    ...
    async def test_list_instances_endpoint(
        ...
        request_data_2 = {**sample_request_data, "learner_id": sample_request_data["learner_id"] + 1}
    ...
    @pytest.mark.asyncio
    async def test_ready_at_timestamp(
        self, 
        db_session: AsyncSession, 
        sample_request_data: dict
    ):
