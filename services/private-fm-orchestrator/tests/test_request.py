...
from app.models import PrivateBrainInstance, PrivateBrainRequest, PrivateBrainStatus
...
class TestPrivateBrainRequest:
    ...
    @pytest.mark.asyncio
    async def test_create_new_request(
        self, 
        client: AsyncClient, 
    ...
    @pytest.mark.asyncio
    async def test_different_learners_separate_instances(
        self, 
        db_session: AsyncSession
    ):
    ...
    async def test_request_validation(self, client: AsyncClient):
        ...
        response = await client.post(
            "/private-brain/request", 
            json={"learner_id": "invalid"}
        )
        assert response.status_code == 422
    
    @pytest.mark.asyncio
    async def test_request_with_optional_fields(
        self, 
        client: AsyncClient, 
