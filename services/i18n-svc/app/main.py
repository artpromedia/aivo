...
from app.services.translation_service import LocaleService, TranslationService
...
async def list_translations(
    ...
    key: str | None = Query(None, description="Filter by translation key"),
    ...
    return await service.get_translations(
        locale=locale, key=key, approved_only=approved_only, limit=limit
    )


@app.get("/translations/{translation_id}", response_model=TranslationResponse)
async def get_translation(
    translation_id: UUID, service: TranslationService = Depends(get_translation_service)
):
...
async def bulk_import_translations(
    ...
    for locale, translations in translations_data.items():
        for key, value in translations.items():
            try:
                await service.create_translation(key=key, locale=locale, value=value)
                imported_count += 1
...
async def get_locale_stats(
    locale: str, service: LocaleService = Depends(get_locale_service)
):
...
@app.post(
    "/accessibility/audit/{translation_id}", response_model=AccessibilityAuditResponse
)
async def audit_translation(
    translation_id: UUID,
    audit_type: str = Query("automated", description="Audit type"),
    auditor_id: str | None = Query(None, description="Auditor ID"),
    ...
    try:
        result = await service.audit_translation(
            translation_id=translation_id, audit_type=audit_type, auditor_id=auditor_id
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
...
async def bulk_audit_locale(
    ...
    result = await service.bulk_audit_locale(
        locale=locale, audit_type=audit_type, batch_size=batch_size
    )
    return result
...
async def get_audit_history(
    translation_id: UUID | None = Query(None),
    locale: str | None = Query(None),
    ...
    return await service.get_audit_history(
        translation_id=translation_id, locale=locale, limit=limit
    )
...
async def search_translations(
    ...
    return await service.search_translations(query=query, locale=locale, limit=limit)
...
async def get_missing_translations(
    ...
    return await service.get_missing_translations(
        target_locale=locale, reference_locale=reference_locale
    )
...
async def get_completion_analytics(
    ...
    return {
        ...
        "summary": {
            ...
            "fully_complete": len(
                [a for a in analytics if a["completion_percentage"] == 100]
            ),
            "partially_complete": len(
                [a for a in analytics if 0 < a["completion_percentage"] < 100]
            ),
            "empty_locales": len(
                [a for a in analytics if a["completion_percentage"] == 0]
            ),
        },
    }
