...
class TranslationService:
    ...
    async def get_translation(
        self, key: str, locale: str, fallback: bool = True
    ) -> str | None:
        """
        ...
        stmt = (
            ...
            .where(
                and_(
                    ...
                    Translation.is_approved is True,
                    TranslationKey.is_active is True,
                )
            )
        )
    ...
    async def get_translations_bulk(
        self, keys: list[str], locale: str, fallback: bool = True
    ) -> dict[str, str]:
        """
    ...
    async def approve_translation(
        self, translation_id: UUID, reviewer_id: str, quality_score: float | None = None
    ) -> Translation | None:
    ...
    async def get_locale_stats(self, locale: str) -> dict:
        ...
        # Total keys
        total_stmt = select(TranslationKey).where(TranslationKey.is_active is True)
    ...
    async def search_translations(
        self, query: str, locale: str | None = None, limit: int = 50, offset: int = 0
    ) -> tuple[list[dict], int]:
        ...
        # Count total results
        count_stmt = (
            select(TranslationKey.id).join(Translation).where(and_(*conditions))
        )
    ...
    async def get_missing_translations(
        ...
        # Get all active keys
        all_keys_stmt = select(TranslationKey.key).where(
            TranslationKey.is_active is True
        )
...
class LocaleService:
    ...
    async def get_enabled_locales(self) -> list[LocaleConfiguration]:
        ...
        stmt = (
            ...
            .where(LocaleConfiguration.is_enabled is True)
            .order_by(LocaleConfiguration.display_name)
        )
        result = await self.db.execute(stmt)
        return result.scalars().all()

    async def get_locale_config(self, locale: str) -> LocaleConfiguration | None:
        """Get locale configuration by code."""
        stmt = select(LocaleConfiguration).where(LocaleConfiguration.locale == locale)
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()

    async def update_completion_percentage(self, locale: str) -> float | None:
