...
from .audit import AuditService, MongoArchiveService, S3ExportService
...
async def create_message(
    ...
    try:
        ...
        moderation_result = await moderation_service.moderate_message(
            message_data.content, session.moderation_level
        )
        ...
        # Create audit entry
        background_tasks.add_task(audit_service.create_audit_entry, db, message, previous_audit)
...
async def create_parental_control(
    ...
    try:
        ...
        if (
            str(control_data.parent_id) != current_user["user_id"]
...
async def get_parental_control(
    ...
    try:
        # Check permissions
        if str(parent_id) != current_user["user_id"] and current_user["role"] not in [
            "admin",
            "teacher",
        ]:
            raise HTTPException(status_code=403, detail="Insufficient permissions")
        ...
        query = select(ParentalControl).where(
            and_(ParentalControl.parent_id == parent_id, ParentalControl.learner_id == learner_id)
        )
        result = await db.execute(query)
...
async def get_moderation_stats(
    ...
    try:
        ...
        query = select(
            ...
            func.sum(func.case((ChatMessage.status == MessageStatus.APPROVED, 1), else_=0)).label(
                "approved"
            ),
            func.sum(func.case((ChatMessage.status == MessageStatus.BLOCKED, 1), else_=0)).label(
                "blocked"
            ),
            func.sum(func.case((ChatMessage.status == MessageStatus.FLAGGED, 1), else_=0)).label(
                "flagged"
            ),
            func.avg(ModerationLog.processing_time_ms).label("avg_processing_time"),
            func.count(ModerationLog.id).label("moderation_calls"),
            func.sum(func.case((ChatMessage.contains_pii is True, 1), else_=0)).label(
                "pii_detections"
            ),
        ).select_from(ChatMessage.__table__.join(ModerationLog.__table__, isouter=True))
    ...
    try:
        ...
        active_result = await db.execute(
            select(func.count()).select_from(
                base_query.where(ChatSession.is_active is True).subquery()
            )
        )
        active_sessions = active_result.scalar()

        # Sessions by type
        type_query = select(ChatSession.chat_type, func.count(ChatSession.id)).group_by(
