...
class MerkleTree:
    ...
    def create_block_hash(
        message_id: str, content_hash: str, previous_hash: Optional[str], timestamp: datetime
    ) -> str:
...
class AuditService:
    ...
    def __init__(
        self, s3_service: S3ExportService, mongo_service: Optional[MongoArchiveService] = None
    ):
    ...
    async def create_audit_entry(
        self, db_session, message: ChatMessage, previous_entry: Optional[AuditEntry] = None
    ) -> AuditEntry:
        ...
        try:
            ...
            block_hash = self.merkle_tree.create_block_hash(
                str(message.id), data_hash, previous_hash, timestamp
            )
    ...
    async def archive_session(
        self, db_session, session_id: UUID, messages: List[ChatMessage]
    ) -> bool:
