...
class TestEnrollmentAudit:
    ...
    async def test_enrollment_metadata_includes_context(
        self, db_session, district_allocation, sample_learner_profile
    ):
        ...
        request = EnrollmentRequest(
            learner_profile=LearnerProfile(**sample_learner_profile), context=context
        )
...
class TestEnrollmentEvents:
    ...
    @pytest.mark.asyncio
    async def test_event_service_creates_correct_event(self, enrollment_decision):
...
class TestEnrollmentQueryAndReporting:
    ...
    @pytest.mark.asyncio
    async def test_get_enrollment_decision_by_id(self, db_session, enrollment_decision):
    ...
    async def test_enrollment_decisions_have_timestamps(
        ...
        before_enrollment = datetime.now(UTC)
        ...
        after_enrollment = datetime.now(UTC)
        ...
        time_diff_start = abs(
            (decision.created_at.replace(tzinfo=None) - before_enrollment).total_seconds()
        )
        time_diff_end = abs(
            (decision.created_at.replace(tzinfo=None) - after_enrollment).total_seconds()
        )
