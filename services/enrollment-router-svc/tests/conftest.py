...
def sample_district_context():
    ...
    return {"tenant_id": 1, "source": "district_portal"}


@pytest.fixture
def sample_parent_context():
    """Sample parent context for testing."""
    return {"guardian_id": "guardian_123", "source": "parent_portal"}
...
async def district_allocation(db_session: AsyncSession):
    ...
    allocation = DistrictSeatAllocation(
        tenant_id=1, total_seats=100, reserved_seats=10, used_seats=20
    )
...
async def enrollment_decision(db_session: AsyncSession):
    ...
    decision = EnrollmentDecision(
        ...
        learner_profile={"email": "test@example.com", "first_name": "Test", "last_name": "User"},
        tenant_id=1,
