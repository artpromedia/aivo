...
from .models import DistrictSeatAllocation, EnrollmentDecision, EnrollmentStatus, ProvisionSource
from .schemas import (
...
class DistrictSeatService:
        ...
        result = await db.execute(
            select(DistrictSeatAllocation).where(
                and_(
                    ...
                    DistrictSeatAllocation.is_active,
                )
            )
        )
    ...
    async def has_available_seats(
        self, db: AsyncSession, tenant_id: int, seats_needed: int = 1
    ) -> bool:
    ...
    async def reserve_seats(
        self, db: AsyncSession, tenant_id: int, seats_to_reserve: int = 1
    ) -> bool:
    ...
    async def create_allocation(
        self, db: AsyncSession, tenant_id: int, total_seats: int
    ) -> DistrictSeatAllocation:
        """Create new district seat allocation."""
        allocation = DistrictSeatAllocation(tenant_id=tenant_id, total_seats=total_seats)
        db.add(allocation)
        await db.commit()
        await db.refresh(allocation)
        return allocation
...
class PaymentService:
    ...
    def __init__(self, payment_service_url: str = "http://localhost:8001") -> None:
        self.payment_service_url = payment_service_url

    async def create_checkout_session(
        self, guardian_id: str, learner_profile: LearnerProfile, context: EnrollmentContext
    ) -> dict[str, Any]:
...
class EventService:
    ...
    async def publish_enrollment_decision(self, decision: EnrollmentDecision) -> None:
...
class EnrollmentRouterService:
    ...
    async def route_enrollment(
        self, db: AsyncSession, request: EnrollmentRequest
        ...
        try:
            ...
            if request.context.tenant_id:
                result = await self._process_district_enrollment(db, request, decision)
            else:
                result = await self._process_parent_enrollment(db, request, decision)
    ...
    async def _process_district_enrollment(
        self, db: AsyncSession, request: EnrollmentRequest, decision: EnrollmentDecision
    ) -> DistrictEnrollmentResult:
        """Process district-provisioned enrollment."""
        tenant_id = request.context.tenant_id

        # Check if district has available seats
        has_seats = await self.district_service.has_available_seats(db, tenant_id, 1)

        if has_seats:
            # Reserve seat for district enrollment
            reserved = await self.district_service.reserve_seats(db, tenant_id, 1)
            ...
            if reserved:
                ...
                decision.decision_metadata = {"allocation_id": allocation.id, "seats_reserved": 1}
        ...
        logger.info(
            f"No district seats available for tenant {tenant_id}, falling back to parent payment"
        )
    ...
    async def _process_parent_enrollment(
        ...
        try:
            ...
            decision.decision_metadata = {"checkout_session": checkout_result}
