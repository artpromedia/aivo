...
from sqlalchemy import (
    Boolean, DateTime, Enum, Integer, String, Text, JSON, 
    func, ForeignKey
)
...
class EnrollmentDecision(Base):
    ...
    # Learner information
    learner_id: Mapped[Optional[str]] = mapped_column(String(255), index=True)
    learner_email: Mapped[Optional[str]] = mapped_column(String(255), index=True)
    learner_profile: Mapped[dict] = mapped_column(JSON)
    
    # Context information
    tenant_id: Mapped[Optional[int]] = mapped_column(Integer, index=True)
    guardian_id: Mapped[Optional[str]] = mapped_column(String(255), index=True)
    context: Mapped[dict] = mapped_column(JSON)
    
    # Decision results
    provision_source: Mapped[ProvisionSource] = mapped_column(
    ...
    district_seats_available: Mapped[Optional[int]] = mapped_column(Integer)
    district_seats_reserved: Mapped[Optional[int]] = mapped_column(Integer)
    
    # Parent enrollment details
    checkout_session_id: Mapped[Optional[str]] = mapped_column(String(255))
