...
from app.enums import ApprovalStatus, ApprovalType, DecisionType, ParticipantRole
from app.models import Approval
from app.schemas import ApprovalCreateInput
...
class TestApprovalCreation:
    ...
    def test_create_approval_invalid_participants(self, client: TestClient, sample_approval_data):
        ...
        sample_approval_data["participants"] = [
            p for p in sample_approval_data["participants"] if p["role"] != ParticipantRole.GUARDIAN
        ]
...
class TestApprovalRetrieval:
    ...
    def test_get_approval_with_tenant_filter(self, client: TestClient, created_approval):
        ...
        # Wrong tenant
        response = client.get(f"/approvals/{created_approval.id}?tenant_id=wrong_tenant")
        assert response.status_code == 404
...
class TestDecisionMaking:
    ...
    def test_make_decision_approve(self, client: TestClient, pending_approval):
        ...
        decision_data = {"decision_type": DecisionType.APPROVE, "comments": "Looks good to me"}
    ...
    def test_make_decision_reject(self, client: TestClient, pending_approval):
        ...
        participant = pending_approval.participants[0]
        decision_data = {"decision_type": DecisionType.REJECT, "comments": "Needs revision"}
...
class TestExpirationFlow:
    ...
    async def expired_approval(self, db_session: AsyncSession, sample_approval_data):
        ...
        db_approval.expires_at = datetime.now(UTC) - timedelta(hours=1)
...
class TestPermissions:
    ...
    def test_tenant_isolation(self, client: TestClient, tenant_approval):
        ...
        response = client.get(f"/approvals/{tenant_approval.id}?tenant_id=specific_tenant")
        assert response.status_code == 200

        # Access with wrong tenant
        response = client.get(f"/approvals/{tenant_approval.id}?tenant_id=wrong_tenant")
        assert response.status_code == 404
