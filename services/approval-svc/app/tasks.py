...
def send_approval_notification_task(self, approval_id: str, participant_id: str):
    ...
    try:
        ...
        if success:
            logger.info(
                f"Sent approval notification for {approval_id} to participant {participant_id}"
            )
        else:
            logger.error(
                f"Failed to send approval notification for {approval_id} to participant {participant_id}"
            )
...
def schedule_approval_notifications(approval_id: str, participant_ids: list[str]) -> None:
...
def schedule_completion_notification(approval_id: str) -> None:
