...
from .models import Approval, ApprovalParticipant, ApprovalDecision
from .schemas import (
    ApprovalCreateInput, ApprovalResponse, ApprovalSummary, 
    ApprovalListQuery, DecisionInput, DecisionResult
)
from .enums import ApprovalStatus, DecisionType, WebhookEventType
...
class ApprovalService:
    ...
    async def list_approvals(
        ...
        stmt = stmt.options(
            selectinload(Approval.participants),
    ...
    async def make_decision(
        ...
        try:
            ...
            logger.info(f"Decision {decision.decision_type.value} made by {user_id} for approval {approval.id}")
    ...
    async def expire_approvals(self, db: AsyncSession) -> int:
        ...
        for approval in expired_approvals:
            ...
            if approval.webhook_url:
                await webhook_service.send_webhook(
                    approval,
                    WebhookEventType.APPROVAL_EXPIRED
                )
    ...
    async def _check_approval_completion(self, db: AsyncSession, approval: Approval) -> bool:
        ...
        if rejected_count > 0:
            ...
            if approval.webhook_url:
                await webhook_service.send_webhook(
                    approval,
                    WebhookEventType.APPROVAL_REJECTED
                )
            ...
            if approval.webhook_url:
                await webhook_service.send_webhook(
                    approval,
                    WebhookEventType.APPROVAL_COMPLETED
                )
