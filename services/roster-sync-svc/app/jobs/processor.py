...
celery_app = Celery(
    "roster-sync", broker="redis://localhost:6379/0", backend="redis://localhost:6379/0"
)
        ...
        try:
            ...
            if not connection_test.get("success"):
                raise ConnectorError(
                    f"Connection test failed: {connection_test.get('message')}"
                )
            ...
            # Calculate totals
            total_processed = sum(
                r.get("processed", 0) for r in results.values() if isinstance(r, dict)
            )
            total_created = sum(
                r.get("created", 0) for r in results.values() if isinstance(r, dict)
            )
            total_updated = sum(
            ...
            logger.info(
                f"Job {self.job_id} completed successfully: {total_processed} records processed"
            )
            return final_result
            ...
            # Store error information for logging
            _error_result = {
                "job_id": self.job_id,
...
def test_connector(
    connector_type: str, connector_config: dict[str, Any]
) -> dict[str, Any]:
    """
...
def cleanup_old_jobs(days_old: int = 30) -> dict[str, Any]:
    ...
    return {"cleaned_jobs": 0, "cleaned_logs": 0, "days_old": days_old}


@celery_app.task(name="roster_sync.send_webhook_notification")
def send_webhook_notification(
    webhook_url: str, webhook_secret: str, payload: dict[str, Any]
) -> bool:
    """
    ...
    try:
        ...
        signature = hmac.new(
            webhook_secret.encode("utf-8"), payload_str, hashlib.sha256
        ).hexdigest()
