...
from app.jobs.processor import celery_app, sync_roster_data, test_connector
...
class ConnectorConfigModel(BaseModel):
    ...
    credentials: dict[str, Any] | None = Field(
        None, description="Connector credentials"
    )
...
class JobStatusResponse(BaseModel):
    ...
    total_processed: int | None = Field(None, description="Total records processed")
    total_created: int | None = Field(None, description="Total records created")
    total_updated: int | None = Field(None, description="Total records updated")
    total_failed: int | None = Field(None, description="Total records failed")
...
async def test_connector_config(config: ConnectorConfigModel) -> ConnectorTestResponse:
    ...
    try:
        ...
        return ConnectorTestResponse(
            success=False, message=f"Test failed: {str(e)}", details={"error": str(e)}
        )
...
async def create_sync_job(job_request: SyncJobRequest) -> SyncJobResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create sync job: {str(e)}"
        ) from e
...
async def get_connector_stats() -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get connector stats: {str(e)}"
        ) from e
...
async def get_scim_status(district_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get SCIM status: {str(e)}"
        ) from e
