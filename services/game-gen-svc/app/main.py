...
from app.models import (
    ManifestRequest, ManifestResponse, CacheStats,
    HealthResponse, PerformanceStats, AccessibilitySettings
)
...
async def generate_game_manifest(
    ...
    try:
        # Use grade_level or default to grade 1
        grade = request.grade_level or 1
        
        # Use provided accessibility or defaults
        accessibility = request.accessibility or AccessibilitySettings()
        
        # Validate request parameters
        if request.minutes < settings.min_game_duration_minutes:
            raise HTTPException(
                status_code=400,
                detail=f"Duration must be at least {settings.min_game_duration_minutes} minutes"
            )
        
        if request.minutes > settings.max_game_duration_minutes:
            raise HTTPException(
                status_code=400,
                detail=f"Duration cannot exceed {settings.max_game_duration_minutes} minutes"
            )
...
async def warm_cache(
    ...
    try:
        ...
        return {"message": f"Cache warming started for {subject} grade {grade}"}
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
async def get_available_games(subject: str, grade: int = 1):
