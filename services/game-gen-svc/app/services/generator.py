...
from app.models import (
    GameManifest, GameScene, GameAsset, ScoringConfig,
    SubjectType, GameType, DifficultyLevel, AccessibilitySettings
)
...
class GameGenerationService:
    ...
    async def generate_manifest(
        ...
        try:
            ...
            manifest = GameManifest(
                ...
                description=f"A {difficulty.value} level {subject.value} game for grade {grade}",
                scenes=scenes,
                scoring=scoring,
    ...
    def _calculate_scene_count(self, duration_minutes: int, 
    ...
    async def _generate_scene(self, subject: SubjectType, game_type: GameType,
                            difficulty: DifficultyLevel, scene_index: int,
                            accessibility: AccessibilitySettings,
                            duration_minutes: int) -> GameScene:
        ...
        scene_duration = max(30, duration_minutes * 60 // self._calculate_scene_count(duration_minutes, difficulty))
    ...
    def _generate_scene_content(self, subject: SubjectType, game_type: GameType,
                              difficulty: DifficultyLevel, scene_index: int) -> tuple[str, str]:
    ...
    def _generate_scene_assets(
        ...
        bg_asset = GameAsset(
            asset_id="background",
            asset_type="image",
            url=f"{settings.assets_base_url}/backgrounds/{subject.value}_bg.jpg",
        ...
        if game_type == GameType.PUZZLE:
            for i in range(6):
                piece_asset = GameAsset(
                    ...
                    asset_type="image",
                    url=f"{settings.assets_base_url}/puzzle/{subject.value}_piece_{i}.png",
            ...
            if accessibility.audio_cues:
                audio_asset = GameAsset(
                    ...
                    asset_type="audio",
                    url=f"{settings.assets_base_url}/audio/{subject.value}_question.mp3",
            ...
            for i in range(4):
                card_asset = GameAsset(
                    ...
                    asset_type="image",
                    url=f"{settings.assets_base_url}/cards/{subject.value}_card_{i}.png",
    ...
    def _calculate_required_interactions(self, game_type: GameType,
                                       difficulty: DifficultyLevel) -> int:
    ...
    def _generate_scoring_config(self, subject: SubjectType, game_type: GameType,
        ...
        return ScoringConfig(
            max_points=base_points[difficulty] * 10,  # Max points for perfect game
    ...
    def _apply_accessibility_adaptations(
        ...
        for scene in scenes:
            ...
            if accessibility.simplified_ui:
                adapted_scene.description = self._simplify_instructions(scene.description)
    ...
    def get_performance_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "performance_ratio": (avg_generation_time * 1000) / settings.target_generation_time_ms if avg_generation_time > 0 else 0
        }
