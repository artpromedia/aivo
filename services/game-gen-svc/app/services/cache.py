...
from app.models import GameManifest, CacheStats
...
class TemplateCache:
    ...
    async def connect(self) -> None:
        ...
        if settings.cache_enabled:
            try:
                ...
                print(f"Redis connection failed: %s", str(e))
                self.redis = None
    ...
    def _generate_cache_key(self, subject: str, grade: int, 
                          game_type: str, duration_minutes: int,
                          accessibility_hash: str) -> str:
        """Generate cache key for game manifest."""
        return f"manifest:{subject}:{grade}:{game_type}:{duration_minutes}:{accessibility_hash}"
    ...
    async def get_manifest(self, subject: str, grade: int, 
                          game_type: str, duration_minutes: int,
                          accessibility: Dict[str, Any]) -> Optional[GameManifest]:
        ...
        async with self._cache_lock:
            ...
            if self.redis:
                try:
                    ...
                    print(f"Redis get error: %s", str(e))
            
            # Cache miss
            self.cache_stats["misses"] += 1
            return None
    
    async def store_manifest(self, subject: str, grade: int,
                           game_type: str, duration_minutes: int,
                           accessibility: Dict[str, Any],
                           manifest: GameManifest) -> None:
        ...
        if data_size > settings.cache_max_manifest_size_kb * 1024:
            print(f"Manifest too large for cache: %d bytes", data_size)
            return
        ...
        async with self._cache_lock:
            ...
            if self.redis:
                try:
                    ...
                    print(f"Redis set error: %s", str(e))
            
            # Update stats
            self.cache_stats["size_bytes"] += data_size
            self.cache_stats["num_entries"] += 1
    
    async def get_template_variants(self, subject: str, 
        ...
        if self.redis:
            try:
                ...
                print(f"Redis keys error: %s", str(e))
    ...
    async def warm_cache(self, subject: str, grade: int) -> None:
        ...
        print(f"Cache warming identified %d variants to generate", len(tasks))
    ...
    async def clear_expired(self) -> int:
        ...
        async with self._cache_lock:
            ...
            for key, data in self.local_cache.items():
                if current_time - data.get("created_at", 0) > settings.cache_default_ttl_seconds:
