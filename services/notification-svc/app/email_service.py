...
class EmailService:
    ...
    def __init__(self):
        ...
        if self.dev_mode:
            ...
            logger.info(f"Email service in development mode - emails will be saved to {self.dev_email_path}")
        else:
    ...
    async def _save_dev_email(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            filename = f"email_{timestamp}_{email_data['to'].replace('@', '_at_').replace('.', '_')}.json"
            ...
            return {
                "success": False,
                "error": f"Failed to save development email: {e}"
            }
