...
from .models import Seat, SeatAudit, SeatState, Tenant, TenantKind, UserTenantRole
from .schemas import (
    DistrictCreate,
    SchoolCreate,
    SeatAllocate,
...
class TenantService:
    ...
    async def create_district(db: AsyncSession, district_data: DistrictCreate) -> Tenant:
        ...
        tenant_data = TenantCreate(
            name=district_data.name, kind=TenantKind.DISTRICT, parent_id=None
        )
    ...
    async def create_school(
        db: AsyncSession, district_id: int, school_data: SchoolCreate
    ) -> Tenant:
        ...
        tenant_data = TenantCreate(
            name=school_data.name, kind=TenantKind.SCHOOL, parent_id=district_id
        )
    ...
    @staticmethod
    async def get_district_with_schools(db: AsyncSession, district_id: int) -> Tenant | None:
    ...
    async def update_tenant(
        db: AsyncSession, tenant_id: int, tenant_data: TenantUpdate
    ) -> Tenant | None:
...
class SeatService:
    ...
    @staticmethod
    async def allocate_seat(db: AsyncSession, allocation_data: SeatAllocate, user_id: str) -> Seat:
    ...
    @staticmethod
    async def reclaim_seat(db: AsyncSession, reclaim_data: SeatReclaim, user_id: str) -> Seat:
        ...
        if seat.state == SeatState.FREE:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Seat is already free"
            )
    ...
    async def get_learner_seat_in_tenant(
        db: AsyncSession, learner_id: str, tenant_id: int
    ) -> Seat | None:
    ...
    @staticmethod
    async def get_seat_summary(db: AsyncSession, tenant_id: int) -> SeatSummary | None:
        ...
        result = await db.execute(
            select(Seat.state, func.count(Seat.id).label("count"))
        ...
        utilization_percentage = (
            ((reserved_seats + assigned_seats) / total_seats * 100) if total_seats > 0 else 0
        )
...
class UserRoleService:
    ...
    @staticmethod
    async def assign_user_role(db: AsyncSession, role_data: UserTenantRoleCreate) -> UserTenantRole:
    ...
    async def get_user_role(
        db: AsyncSession, user_id: str, tenant_id: int
    ) -> UserTenantRole | None:
        """Get user's role in a specific tenant."""
        result = await db.execute(
            select(UserTenantRole).where(
                and_(UserTenantRole.user_id == user_id, UserTenantRole.tenant_id == tenant_id)
            )
        )
        return result.scalar_one_or_none()
    ...
    async def get_user_tenants(db: AsyncSession, user_id: str) -> list[UserTenantRole]:
        ...
        result = await db.execute(
            ...
            .where(and_(UserTenantRole.user_id == user_id, UserTenantRole.is_active))
        )
