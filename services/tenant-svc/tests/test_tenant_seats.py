...
from app.models import Base, SeatState, TenantKind
...
class TestTenantLifecycle:
    ...
    async def test_create_district(self, test_client: AsyncClient, test_user_id: str):
        ...
        response = await test_client.post(
            "/api/v1/district", json=district_data, headers={"X-User-ID": test_user_id}
        )
    ...
    async def test_get_district_with_schools(self, test_client: AsyncClient, test_user_id: str):
        ...
        response = await test_client.get(
            f"/api/v1/district/{district_id}", headers={"X-User-ID": test_user_id}
        )
    ...
    async def test_create_school_invalid_district(
        self, test_client: AsyncClient, test_user_id: str
    ):
        """Test creating school with invalid district ID."""
        school_data = {"name": "Invalid School"}

        response = await test_client.post(
            "/api/v1/district/99999/schools", json=school_data, headers={"X-User-ID": test_user_id}
        )
...
class TestSeatLifecycle:
    ...
    async def test_purchase_seats(self, test_client: AsyncClient, test_user_id: str):
        ...
        response = await test_client.post(
            "/api/v1/seats/purchase", json=seat_data, headers={"X-User-ID": test_user_id}
        )
    ...
    async def test_allocate_seat(self, test_client: AsyncClient, test_user_id: str):
        ...
        await self.setup_tenant(test_client, test_user_id)
        seat_ids = await self.test_purchase_seats(test_client, test_user_id)

        allocation_data = {"seat_id": seat_ids[0], "learner_id": "learner_123"}

        response = await test_client.post(
            "/api/v1/seats/allocate", json=allocation_data, headers={"X-User-ID": test_user_id}
        )
    ...
    async def test_reclaim_seat(self, test_client: AsyncClient, test_user_id: str):
        ...
        reclaim_data = {"seat_id": allocated_seat["id"], "reason": "Student graduated"}

        response = await test_client.post(
            "/api/v1/seats/reclaim", json=reclaim_data, headers={"X-User-ID": test_user_id}
        )
    ...
    async def test_seat_summary(self, test_client: AsyncClient, test_user_id: str):
        ...
        # Allocate some seats
        allocation_data = {"seat_id": seat_ids[0], "learner_id": "learner_1"}
        await test_client.post(
            "/api/v1/seats/allocate", json=allocation_data, headers={"X-User-ID": test_user_id}
        )

        allocation_data = {"seat_id": seat_ids[1], "learner_id": "learner_2"}
        await test_client.post(
            "/api/v1/seats/allocate", json=allocation_data, headers={"X-User-ID": test_user_id}
        )
        ...
        # Get summary
        response = await test_client.get(
            f"/api/v1/seats/summary?tenantId={tenant_id}", headers={"X-User-ID": test_user_id}
        )
    ...
    async def test_idempotent_allocation(self, test_client: AsyncClient, test_user_id: str):
        ...
        # Allocate first seat
        allocation_data = {"seat_id": seat_ids[0], "learner_id": "learner_duplicate"}
        ...
        allocation_data = {"seat_id": seat_ids[1], "learner_id": "learner_duplicate"}
    ...
    async def test_allocate_unavailable_seat(self, test_client: AsyncClient, test_user_id: str):
        ...
        allocation_data = {"seat_id": allocated_seat["id"], "learner_id": "another_learner"}
...
class TestErrorHandling:
    ...
    async def test_purchase_seats_invalid_tenant(self, test_client: AsyncClient, test_user_id: str):
        ...
        seat_data = {"tenant_id": 99999, "count": 5}
    ...
    async def test_allocate_nonexistent_seat(self, test_client: AsyncClient, test_user_id: str):
        ...
        allocation_data = {"seat_id": 99999, "learner_id": "learner_123"}
    ...
    async def test_get_nonexistent_tenant(self, test_client: AsyncClient, test_user_id: str):
        ...
        response = await test_client.get(
            "/api/v1/tenant/99999", headers={"X-User-ID": test_user_id}
        )

        assert response.status_code == 404

    async def test_seat_summary_nonexistent_tenant(
        self, test_client: AsyncClient, test_user_id: str
    ):
        """Test seat summary for non-existent tenant."""
        response = await test_client.get(
            "/api/v1/seats/summary?tenantId=99999", headers={"X-User-ID": test_user_id}
        )
...
class TestUserRoles:
    ...
    async def test_assign_user_role(self, test_client: AsyncClient, test_user_id: str):
        ...
        # Assign role
        role_data = {"user_id": "teacher_123", "tenant_id": tenant_id, "role": "teacher"}

        response = await test_client.post(
            "/api/v1/roles", json=role_data, headers={"X-User-ID": test_user_id}
        )
    ...
    async def test_get_user_tenants(self, test_client: AsyncClient, test_user_id: str):
        ...
        response = await test_client.get(
            "/api/v1/users/teacher_123/tenants", headers={"X-User-ID": test_user_id}
        )
...
class TestIntegrationScenarios:
    ...
    async def test_complete_district_school_seat_workflow(
        ...
        for school_id in [school1_id, school2_id]:
            response = await test_client.get(
                f"/api/v1/seats/summary?tenantId={school_id}", headers={"X-User-ID": test_user_id}
            )
            assert response.status_code == 200
