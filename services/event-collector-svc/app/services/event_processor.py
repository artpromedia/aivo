...
class EventProcessor:
                ...
                try:
                    ...
                    if not events:
                        ...
                        await self.buffer.remove_batch(
                            batch["file_path"], batch["batch_id"]
                        )
                        continue
                    ...
                    if successful > 0 and failed == 0:
                        ...
                        self._metrics["last_processing_time"] = (
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            kafka_healthy = (
                kafka_health.get("kafka", {}).get("status") == "healthy"
            )
            processor_healthy = (
                self._running
            ...
            overall_status = (
                "healthy"
                if kafka_healthy and processor_healthy
                else "unhealthy"
            )
            ...
            return {
                ...
                "buffer": {
                    ...
                    "oldest_event": (
                        buffer_stats.oldest_event.isoformat()
                        if buffer_stats.oldest_event
                        else None
                    ),
                    "newest_event": (
                        buffer_stats.newest_event.isoformat()
                        if buffer_stats.newest_event
                        else None
                    ),
                },
                **kafka_health,
