...
class Settings(BaseSettings):
    ...
    tenant_service_url: str = Field(
        default="http://localhost:8080", description="Tenant service URL"
    )
    payment_service_url: str = Field(
        default="http://localhost:8085", description="Payment service URL"
    )
    approval_service_url: str = Field(
        default="http://localhost:8089", description="Approval service URL"
    )
    analytics_service_url: str = Field(
        default="http://localhost:8096", description="Analytics service URL"
    )
    fm_orchestrator_url: str = Field(
        default="http://localhost:8097", description="Private FM Orchestrator URL"
    )
    ...
    circuit_breaker_expected_exception: str = Field(
        default="httpx.RequestError", description="Expected exception for circuit breaker"
    )
    ...
    otel_service_name: str = Field(
        default="admin-portal-svc", description="OpenTelemetry service name"
    )
    ...
    model_config = ConfigDict(env_file=".env", env_prefix="ADMIN_PORTAL_")
