...
from .http_client import http_client, CircuitBreakerError
from .cache_service import cache_service
from .schemas import (
    SummaryResponse, SubscriptionDetails, BillingHistoryResponse,
    TeamResponse, UsageResponse, NamespacesResponse,
    TenantStatus, SubscriptionTier, UsageMetric, BillingHistoryItem,
    TeamMember, NamespaceInfo
)
...
class ServiceAggregator:
    ...
    async def _get_cached_or_fetch(
        ...
        try:
            ...
            logger.error(f"Failed to fetch data for {endpoint}: {e}")
        ...
        async def fetch_summary():
            ...
            for key, task in tasks.items():
                try:
                    ...
                    logger.warning(f"Failed to fetch {key} for summary: {e}")
                    results[key] = {}
    ...
    async def get_subscription(self, tenant_id: str) -> SubscriptionDetails:
        ...
        async def fetch_subscription():
            subscription_data = await http_client.get(
                "tenant_service",
                f"{self.settings.tenant_service_url}/tenants/{tenant_id}/subscription"
            )
            ...
            try:
                payment_data = await http_client.get(
                    "payment_service",
                    f"{self.settings.payment_service_url}/subscriptions/{tenant_id}"
                )
                subscription_data.update(payment_data)
            except Exception as e:
                logger.warning(f"Failed to get payment data: {e}")
    ...
    async def get_billing_history(self, tenant_id: str) -> BillingHistoryResponse:
        ...
        async def fetch_billing():
            billing_data = await http_client.get(
                "payment_service",
                f"{self.settings.payment_service_url}/billing/{tenant_id}/history"
            )
    ...
    async def get_team(self, tenant_id: str) -> TeamResponse:
        ...
        async def fetch_team():
            team_data = await http_client.get(
                "tenant_service",
                f"{self.settings.tenant_service_url}/tenants/{tenant_id}/team"
            )
    ...
    async def get_usage(self, tenant_id: str) -> UsageResponse:
        ...
        async def fetch_usage():
            # Get usage from analytics service (when available) or tenant service
            try:
                usage_data = await http_client.get(
                    "analytics_service",
                    f"{self.settings.analytics_service_url}/usage/{tenant_id}"
                )
            except CircuitBreakerError:
                # Fallback to tenant service
                usage_data = await http_client.get(
                    "tenant_service",
                    f"{self.settings.tenant_service_url}/tenants/{tenant_id}/usage"
                )
    ...
    async def get_namespaces(self, tenant_id: str) -> NamespacesResponse:
        ...
        async def fetch_namespaces():
            namespaces_data = await http_client.get(
                "fm_orchestrator",
                f"{self.settings.fm_orchestrator_url}/tenants/{tenant_id}/namespaces"
            )
            ...
            return {
                ...
                "storage_distribution": {
                    ...
                    for ns in namespaces if ns["storage_used_mb"] > 0
                }
            }
    ...
    async def _get_user_stats(self, tenant_id: str) -> Dict[str, Any]:
        ...
        return await http_client.get(
            "tenant_service",
            f"{self.settings.tenant_service_url}/tenants/{tenant_id}/users/stats"
        )
    ...
    async def _get_document_stats(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            return await http_client.get(
                "fm_orchestrator",
                f"{self.settings.fm_orchestrator_url}/tenants/{tenant_id}/documents/stats"
            )
        except:
            return {"total": 0}
    ...
    async def _get_approval_stats(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            return await http_client.get(
                "approval_service",
                f"{self.settings.approval_service_url}/approvals/stats?tenant_id={tenant_id}"
            )
        except:
            return {"pending": 0}
    ...
    async def _get_billing_summary(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            return await http_client.get(
                "payment_service",
                f"{self.settings.payment_service_url}/billing/{tenant_id}/summary"
            )
        except:
    ...
    async def _get_usage_alerts(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            return await http_client.get(
                "tenant_service",
                f"{self.settings.tenant_service_url}/tenants/{tenant_id}/alerts"
            )
        except:
            return {"count": 0}
