...
from app.crdt_manager import crdt_manager, CrdtOperation
from app.approval_service import approval_service
from app.event_service import event_service
from app.main import app
from app.schema import GoalInput, AccommodationInput
...
def mock_services():
    ...
    with patch.object(event_service, 'publish_event', new_callable=AsyncMock) as mock_event, \
         patch.object(approval_service, 'submit_for_approval', new_callable=AsyncMock) as mock_approval:
...
class TestGraphQLMutations:
    ...
    def test_create_iep_mutation(self, sample_iep_data, sample_goal_data, sample_accommodation_data, mock_services):
        ...
        variables = {
            "input": {
                ...
                "accommodations": [{
                    "accommodationType": sample_accommodation_data["accommodation_type"].value.upper(),
    ...
    def test_save_draft_mutation(self, sample_iep_data, mock_services):
        ...
        assert data["data"]["saveDraft"]["iep"]["presentLevels"] == "Updated present levels via CRDT"
        assert data["data"]["saveDraft"]["iep"]["version"] == 2
    
    def test_submit_for_approval_mutation(self, sample_iep_data, sample_goal_data, sample_accommodation_data, mock_services):
        ...
        goal_input = GoalInput(**sample_goal_data)
        acc_input = AccommodationInput(**sample_accommodation_data)
        ...
        crdt_manager.apply_operation(iep_doc.id, CrdtOperation(
            operation_type="insert",
            path="goals",
            value=goal_json,
            author="teacher_123",
