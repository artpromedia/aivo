...
class ApprovalService:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.base_url}/requests", json=approval_request, timeout=self.timeout
                )
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.base_url}/requests/{approval_request_id}", timeout=self.timeout
                )
                response.raise_for_status()
    ...
    async def _handle_approval_completed(
        self, iep_id: str, approval_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _handle_single_approval(
        ...
        approved_count = sum(
            1 for record in iep_doc.approval_records if record.status == ApprovalStatus.APPROVED
        )
        iep_doc.pending_approval_count = max(0, iep_doc.required_approval_count - approved_count)

        logger.info(
            f"Received approval {approved_count}/{iep_doc.required_approval_count} for IEP {iep_id}"
        )
        return {
            "success": True,
            "message": f"Approval {approved_count}/{iep_doc.required_approval_count} received",
        }
