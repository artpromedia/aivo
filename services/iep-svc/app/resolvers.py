...
from .schema import (
    IepDoc, Goal, Accommodation, IepDocInput, GoalInput, AccommodationInput,
    IepMutationResult, ApprovalResult, CrdtOperation
)
from .enums import IepStatus, ApprovalStatus, EventType
...
class Query:
    ...
    @strawberry.field
    async def iep(self, id: str) -> Optional[IepDoc]:
        """Get an IEP document by ID."""
        return crdt_manager.get_document(id)
    
    @strawberry.field
    async def ieps(self, student_id: Optional[str] = None) -> List[IepDoc]:
    ...
    async def create_iep(
        ...
        try:
            ...
            return IepMutationResult(
                success=True,
                message="IEP document created successfully",
                iep=iep_doc
            )
            ...
            return IepMutationResult(
                success=False,
                message="Failed to create IEP document",
                errors=[str(e)]
            )
    ...
    async def save_draft(
        ...
        try:
            ...
            if not iep_doc:
                return IepMutationResult(
                    success=False,
                    message="IEP document not found",
    ...
    async def submit_for_approval(
        ...
        try:
            ...
            if validation_errors:
                return ApprovalResult(
                    success=False,
                    message="IEP document is incomplete",
...
async def _add_accommodation_to_iep(iep_doc: IepDoc, acc_input: AccommodationInput, added_by: str) -> Accommodation:
