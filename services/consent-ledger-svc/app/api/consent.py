...
from app.models import ConsentStatus, ConsentType
...
class ConsentRequest(BaseModel):
    ...
    user_agent: str | None = Field(None, description="User agent information")
...
class ConsentUpdateRequest(BaseModel):
    ...
    given: bool = Field(..., description="Whether consent is given")
    purposes: list[str] | None = Field(None, description="Updated purposes")
    user_agent: str | None = Field(None, description="User agent information")
    ip_address: str | None = Field(None, description="IP address")
    metadata: dict | None = Field(None, description="Additional metadata")
...
async def create_consent(
    request: ConsentRequest, consent_service: ConsentService = Depends(get_consent_service)
):
    """
...
async def get_user_consents(
    user_id: str,
    consent_type: ConsentType | None = Query(None, description="Filter by consent type"),
    status: ConsentStatus | None = Query(None, description="Filter by status"),
...
async def get_consent(
    consent_id: UUID, consent_service: ConsentService = Depends(get_consent_service)
):
    """
...
async def withdraw_consent(
    consent_id: UUID,
    user_agent: str | None = Query(None, description="User agent"),
    ip_address: str | None = Query(None, description="IP address"),
    consent_service: ConsentService = Depends(get_consent_service),
):
    """
    ...
    try:
        success = await consent_service.withdraw_consent(
            consent_id=consent_id, user_agent=user_agent, ip_address=ip_address
        )
...
async def get_consent_status(
    ...
    try:
        valid_consents = await consent_service.check_consent_validity(
            user_id=user_id, purposes=purposes
        )
