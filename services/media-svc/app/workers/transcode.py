...
class HLSTranscoder:
    ...
    def validate_input(self, file_path: str) -> dict[str, any]:
        ...
        try:
            ...
            raise TranscodingError(f"Failed to probe input file: {e}") from e
        except Exception as e:
            logger.error("Unexpected error during probe: %s", e)
            raise TranscodingError(f"Unexpected error: {e}") from e
                ...
                if current_task:
                    ...
                    current_task.update_state(
                        ...
                        meta={
                            ...
                            "stage": (
                                f"Transcoding "
                                f"{variant_playlists[video_streams.index(stream)]['quality_label']}"
                            ),
                        },
                    )
...
def transcode_to_hls(
    ...
    try:
        ...
        async def update_upload_status(status: str, error: str | None = None) -> None:
        ...
        async def get_upload() -> MediaUpload | None:
            async with AsyncSession(engine) as session:
                result = await session.execute(
                    select(MediaUpload).where(MediaUpload.id == uuid.UUID(upload_id))
                )
                ...
                async with AsyncSession(engine) as session:
                    ...
                    for i, variant in enumerate(quality_variants):
                        hls_output = HLSOutput(
                            ...
                            variant_playlist_s3_key=s3_upload_results["variant_keys"][
                                f"variant_{i}"
                            ],
...
async def upload_hls_to_s3(
    ...
    try:
        ...
        for i, _variant in enumerate(transcoding_results["variant_playlists"]):
        ...
        raise TranscodingError(f"S3 upload failed: {e}") from e
