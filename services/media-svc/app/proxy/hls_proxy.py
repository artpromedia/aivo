...
class PlaylistWhitelistProxy:
    ...
    def verify_access_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HLSProxyError(f"Invalid token: {e}")
    ...
    async def proxy_master_playlist(
        ...
        try:
            ...
            raise HLSProxyError("Failed to fetch playlist")
        except Exception as e:
            logger.error("Master playlist proxy error: %s", e)
            raise HLSProxyError(f"Proxy error: {e}")
    ...
    async def proxy_segment(
        ...
        try:
            ...
            raise HLSProxyError("Failed to fetch segment")
        except Exception as e:
    ...
    def _rewrite_master_playlist(
        ...
        for line in lines:
            if line.startswith("#") or not line.strip():
                ...
                proxied_url = f"{base_url}/api/v1/media/hls/{video_id}/variant/{variant_path}?token={token}"
    ...
    def _rewrite_variant_playlist(
        ...
        for line in lines:
            if line.startswith("#") or not line.strip():
                ...
                proxied_url = f"{base_url}/api/v1/media/hls/{video_id}/segment/{segment_path}?token={token}"
                rewritten_lines.append(proxied_url)
            else:
                # Keep other lines as-is
                rewritten_lines.append(line)
