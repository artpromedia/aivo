...
class HLSOutput(Base):
    ...
    transcoding_completed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    )
    transcoding_error: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class PlaylistWhitelist(Base):
    ...
    ip_range: Mapped[str | None] = mapped_column(String(50), nullable=True)  # CIDR notation
...
class ZoomLTIConfig(Base):
    ...
    zoom_webhook_secret: Mapped[str | None] = mapped_column(String(255), nullable=True)
...
class LiveSession(Base):
    ...
    zoom_password: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    # LTI context
    lti_context_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    lti_resource_link_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
...
class AttendanceRecord(Base):
    ...
    zoom_participant_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    ...
    camera_on_duration: Mapped[int | None] = mapped_column(Integer, nullable=True)
    microphone_on_duration: Mapped[int | None] = mapped_column(Integer, nullable=True)
    chat_messages_count: Mapped[int] = mapped_column(Integer, default=0)
    screen_share_duration: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # Connection quality
    average_connection_quality: Mapped[str | None] = mapped_column(String(20), nullable=True)
    connection_issues_count: Mapped[int] = mapped_column(Integer, default=0)

    # LTI context
    lti_user_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    lti_roles: Mapped[str | None] = mapped_column(String(500), nullable=True)
