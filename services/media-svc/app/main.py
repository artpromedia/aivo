"""FastAPI media service application with video upload, HLS transcoding, and Zoom LTI integration."""
import logging
import uuid
...
from .lti.zoom_integration import ZoomLTIHandler, ZoomLTIError
from .models import AttendanceRecord, HLSOutput, LiveSession, MediaUpload, ZoomLTIConfig
...
HLS_PROXY_SECRET = "your-secret-key-here"  # Should be env var
ZOOM_WEBHOOK_SECRET = "zoom-webhook-secret"  # Should be env var

# Global objects
zoom_lti_handler: Optional[ZoomLTIHandler] = None
hls_proxy: Optional[PlaylistWhitelistProxy] = None
...
async def create_lti_config(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to create LTI config")
...
async def handle_lti_launch(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to handle LTI launch")
...
async def create_live_session(
    ...
    try:
        ...
        logger.info("Created live session %s with Zoom meeting %s", live_session.id, zoom_meeting["meeting_id"])
...
async def get_attendance_report(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get attendance report")
