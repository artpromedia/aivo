...
class GradeCalculatorService:
    ...
    @staticmethod
    def calculate_grade_from_dob(dob: date, reference_date: date | None = None) -> int:
        """
    ...
    async def publish_private_brain_request(
        ...
        try:
            ...
            event_record = PrivateBrainRequest(
                learner_id=learner_id, event_type="PRIVATE_BRAIN_REQUEST"
            )
            ...
            event_data = PrivateBrainRequestEvent(
                learner_id=learner_id, timestamp=event_record.created_at
            )
            ...
            logger.error(
                f"Failed to publish PRIVATE_BRAIN_REQUEST event for learner {learner_id}: {e}"
            )
...
class LearnerService:
    ...
    async def get_learner(self, learner_id: int, include_relations: bool = False) -> Learner | None:
        """Get learner by ID."""
    ...
    async def update_learner(self, learner_id: int, update_data: LearnerUpdate) -> Learner | None:
    ...
    async def assign_teacher(
        self, learner_id: int, teacher_id: int, assigned_by: str | None = None
    ) -> bool:
        """Assign a teacher to a learner."""
        try:
            # Verify learner and teacher exist
            learner_result = await self.db.execute(select(Learner).where(Learner.id == learner_id))
            learner = learner_result.scalar_one_or_none()
            if not learner:
                raise ValueError(f"Learner with ID {learner_id} not found")

            teacher_result = await self.db.execute(select(Teacher).where(Teacher.id == teacher_id))
            ...
            existing_result = await self.db.execute(
                select(learner_teacher_association).where(
                    and_(
            ...
            await self.db.execute(
                learner_teacher_association.insert().values(
                    learner_id=learner_id, teacher_id=teacher_id, assigned_by=assigned_by
                )
            )
            await self.db.commit()
    ...
    async def remove_teacher(self, learner_id: int, teacher_id: int) -> bool:
        ...
        try:
            ...
            if result.rowcount > 0:
                ...
                logger.warning(
                    f"No assignment found between teacher {teacher_id} and learner {learner_id}"
                )
                return False
    ...
    async def assign_multiple_teachers(
        self, learner_id: int, teacher_ids: list[int], assigned_by: str | None = None
    ) -> dict[int, bool]:
