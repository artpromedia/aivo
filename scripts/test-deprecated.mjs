...
const lockFile = "test-deprecated-lock.yaml";
const lock = fs.readFileSync(lockFile, "utf8");

// Extract packages from the packages section and dependencies section
const packages = new Set();

// Get packages from the packages section (e.g., "/request@2.88.0:")
...
for (const id of pkgs) {
  try {
    ...
    if (out && out !== "null" && out !== "false" && out !== "undefined" && out !== '""') {
      bad.push({ id, msg: out.replace(/^"|"$/g, '') });
    }
  } catch (e) { 
