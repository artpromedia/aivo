...
import { GraphQLContext, IepDoc, IepDocInput, GoalInput, AccommodationInput, CrdtOperationInput, IepStatus } from '@/types';
...
export const iepResolvers = {
  Query: {
    ...
    pendingApprovals: requireStaff(async (
      _: any,
      { tenantId }: { tenantId?: string },
      context: GraphQLContext
    ) => {
  ...
  Mutation: {
    createIep: requireStaff(async (
      _: any,
      { input }: { input: IepDocInput },
      context: GraphQLContext
    ) => {
      try {
    ...
    submitForApproval: requireStaff(async (
      _: any,
      { iepId }: { iepId: string },
      context: GraphQLContext
    ) => {
    ...
    addAccommodation: requireStaff(async (
      _: any,
      ...
      try {
        ...
        const response = await context.dataSources.iepService.addAccommodation(iepId, accommodation);
