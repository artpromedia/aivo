...
export const learnerResolvers = {
  Query: {
    learner: async (_: any, { id }: { id: string }, context: GraphQLContext) => {
      ...
      // Check tenant access
      const authService = context.user
        ? new (await import('@/middleware/auth')).JWTAuthService({
  ...
  Mutation: {
    createLearner: requireStaff(
      async (_: any, { input }: { input: LearnerInput }, context: GraphQLContext) => {
        ...
        const response = await context.dataSources.learnerService.createLearner(
          learnerWithTenant as any
        );
